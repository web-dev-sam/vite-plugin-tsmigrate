...
| `scoreTypeRisk`    | `boolean`                | `true`                                         | Include type risk in the maintainability score. `false` scores structure only (comprehension + blast) — the type pass still runs and colours nodes, but type errors cost nothing.                      |
